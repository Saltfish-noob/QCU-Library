                ...
                <div class="box-body">
                    ...
                    <div class="table-box">
                            ...
                            <div class="form-group form-inline">
                                <div class="btn-group">
                                    <button type="button" class="btn btn-default" title="新建"
                                            onclick="location.href='${pageContext.request.contextPath}/pages/back-end/user-add.jsp'">
                        ...
                        <form action="${pageContext.request.contextPath}/user/delUser.do" method="post">
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                ...
                                                <a href="${pageContext.request.contextPath}/user/findUserByIdAndAllRole.do?id=${user.id}"
                                                   class="btn bg-olive btn-xs">设置角色</a>
                                            </td>
                                        </tr>
                                    </c:if>
                                </c:forEach>
                            ...
                            <!--数据列表/-->
                            <input type="submit" value="删除用户" class="bg-red pull-right"
                                   onclick="return confirm('该操作将删除所有与该用户相关的信息且不可恢复，我们建议您停用该账户。')">
                        </form>
                    </div>
                    <!-- 数据表格 /-->
                </div>
                            ...
                            <c:choose>
                                <c:when test="${pageInfo.pages<=5}">
                                    <c:forEach begin="1" end="${pageInfo.pages}" var="pageNum">
                                        <li>
                                            <a href="${pageContext.request.contextPath}/user/findAll.do?page=${pageNum}&pageSize=${pageInfo.pageSize}">${pageNum}</a>
                                        </li>
                                    </c:forEach>
                                </c:when>
                                <c:otherwise>
                                    <c:if test="${pageInfo.pageNum+2>pageInfo.pages}">
                                        <c:forEach end="${pageInfo.pageNum-pageInfo.pages+2}" begin="1" step="1"
                                                   var="temp">
                                            <li>
                                                <a href="${pageContext.request.contextPath}/user/findAll.do?page=${pageInfo.pageNum-(5-temp)}&pageSize=${pageInfo.pageSize}">${pageInfo.pageNum-(5-temp)}</a>
                                            </li>
                                        </c:forEach>
                                    </c:if>
                                    ...
                                    <c:if test="${pageInfo.pageNum-2<=0}">
                                        <c:forEach end="${3-pageInfo.pageNum}" begin="1" step="1"
                                                   var="temp">
                                            <li>
                                                <a href="${pageContext.request.contextPath}/user/findAll.do?page=${temp+pageInfo.pageNum+2}&pageSize=${pageInfo.pageSize}">${temp+pageInfo.pageNum+2}</a>
                                            </li>
                                        </c:forEach>
                                    </c:if>
                                </c:otherwise>
